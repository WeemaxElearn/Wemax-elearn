...
class BlogDetailsLeftSidebar extends Component {
	...
	getCertificate() {
		...
		axios
			.get(
				'https://weemaxelearn.in/api/course?id=' +
		...
		axios
			.post('https://weemaxelearn.in/api/printcertificate', {
				userId: data.userId,
  ...
  >
    ...
    >
      ...
      ></div>
					...
					toast.warn(
						'Something went wrong while printing the certificate'
					);
					return;
				}
			})
	...
	onClick(e) {
		...
		if (this.state.buttonclass == 'btn btn-success') {
			axios
				.post(
					'https://weemaxelearn.in/api/enrollbystudent/add',
					newTodo
				)
				.then((result) => {
	...
	async componentDidMount() {
		...
		await axios
			.get(
				'https://weemaxelearn.in/api/checkenrollment?id=' +
					this.state.user +
					'&&courseid=' +
					this.props.match.params.id
			)
				...
				if (result.data != undefined) {
					if (result.data.approved === true) {
						const response = await axios.get(
							'https://weemaxelearn.in/api/lectures?id=' +
								this.props.match.params.id
						);
						this.setState({
